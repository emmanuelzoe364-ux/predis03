...
st.set_page_config(layout="wide", page_title="Revelation Engine Live")

# --- SIDEBAR ---
with st.sidebar:
    st.header("⚙️ Configuration")
    ticker = st.text_input("Primary Ticker", value="BTC-USD").strip().upper()
    timeframe = st.selectbox("Timeframe", ["5m", "15m", "30m", "1h", "1d"], index=0) # Default to 5m
    period = st.selectbox("Data Period", ["1d", "3d", "7d", "1mo"], index=2) # Shorter periods fetch faster
    ...
    st.write(f"Last System Check: {datetime.now().strftime('%H:%M:%S')}")

st.title(f"⚖️ Revelation Engine: {ticker}")
st.caption("Data Mode: Live (30s Cache) | Intermarket Stats")

# --- ROBUST DATA FETCHING ---
@st.cache_data(ttl=30) # Cache reduced to 30 seconds for live updates
def fetch_live_data(main_t, tf, prd):
    try:
        # We fetch using 'prepost=True' to get the absolute latest data points
        m_df = yf.download(main_t, period=prd, interval=tf, auto_adjust=True, prepost=True, progress=False)
        p_df = yf.download("PAXG-USD", period=prd, interval=tf, auto_adjust=True, prepost=True, progress=False)
        b_df = yf.download("BTC-USD", period=prd, interval=tf, auto_adjust=True, prepost=True, progress=False)
        e_df = yf.download("ETH-USD", period=prd, interval=tf, auto_adjust=True, prepost=True, progress=False)
...
df, pb_ratio, be_ratio, err = fetch_live_data(ticker, timeframe, period)
...
if err:
    ...
    # --- ORIGINAL REVELATION P/D LOGIC (-2 to 2) ---
    ...
    # --- DATA TABLE (WITH EMAs) ---
    st.subheader("📋 Live Data Stream (Most Recent at Top)")
    # We display the last 50 candles, sorted newest to oldest
    st.dataframe(df[['Close', 'EMA_30', 'EMA_72', 'H_Val', 'Z_EMA', 'Z_GOLD_BTC', 'Z_BTC_ETH', 'Phase']].sort_index(ascending=False).head(50), use_container_width=True)
